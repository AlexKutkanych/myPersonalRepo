...
import { removeGroup, switchGroupOn, switchGroupOff } from '../api';
...
export default class GroupItem extends PureComponent {
    ...
    handleStateChange = async (e) => {
      const { group, onUpdate } = this.props;
      const { id, devices } = group;
      const newState = e.target.value;

      if (newState === 'on') {
          await switchGroupOn(id, devices);
      } else {
          await switchGroupOff(id, devices);
      }
    ...
    render() {
        ...
        return (
          ...
          <td>{group.name}</td>
          <td>{group.devices.map(({ deviceName }) => <span className="badge badge-pill badge-success mr-2">{deviceName}</span>)}</td>
